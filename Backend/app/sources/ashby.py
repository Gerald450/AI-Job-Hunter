"""Ashby public board listing source."""
...
class AshbyBoardSource:
    """List jobs for one or more Ashby board names."""
    ...
    def __init__(
        ...
        *,
        ...
        self._companies = [
            c for c in companies if c.get("ats") == "ashby" and c.get("enabled", True)
        ]
        self._client = client
    ...
    async def fetch_jobs(self) -> list[Job]:
        ...
        for company in self._companies:
            ...
            try:
                ...
                logger.exception(
                    "Ashby board failed company=%s token=%s",
                    display,
                    token,
                )
        logger.info("Ashby: total jobs=%s boards=%s", len(jobs), len(self._companies))
        return jobs

    async def _fetch_board(self, token: str, company_name: str) -> list[Job]:
        endpoint = f"{_API_BASE}/{token}"
        ...
        if not isinstance(payload, dict):
            raise FetchError("Unsupported Ashby board response", url=endpoint)

        raw_jobs = payload.get("jobs")
        if not isinstance(raw_jobs, list):
            raise FetchError("Ashby board missing jobs list", url=endpoint)
        ...
        for item in raw_jobs:
            ...
            try:
                ...
                logger.exception(
                    "Ashby: skip job company=%s id=%s",
                    company_name,
                    item.get("id"),
                )
        logger.info(
            "Ashby board=%s company=%s jobs=%s",
            token,
            company_name,
            len(jobs),
        )
    ...
    def _to_job(self, item: dict[str, Any], company_name: str, token: str) -> Job:
        ...
        if isinstance(loc, str) and loc.strip():
            location = loc.strip()
        elif isinstance(item.get("locationName"), str) and item["locationName"].strip():
            location = item["locationName"].strip()
        elif isinstance(item.get("address"), dict):
            postal = item["address"].get("postalAddress")
            if isinstance(postal, str) and postal.strip():
                location = postal.strip()

        posted = parse_posted_at(item.get("publishedAt"))
